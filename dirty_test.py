...
def scaler(sensitivity_scores, scaling_factor):
    scaled_alphas_list = []
    scaled_sizes_list = []
    ...
    return scaled_alphas_list, scaled_sizes_list
...
with h5py.File(h5_file, "r") as f:
    ...
    total_steps = config.get("max_epochs") * 2
    ...
    # for the dirty exp use dirty_ and clean_
    xx = [f[f"clean_scores/epoch_{step}"]["decision_boundary"]["xx"][:] for step in range(total_steps)]
    yy = [f[f"clean_scores/epoch_{step}"]["decision_boundary"]["yy"][:] for step in range(total_steps)]
    Z = [f[f"clean_scores/epoch_{step}"]["decision_boundary"]["Z"][:] for step in range(total_steps)]

    # Sensitivity scores (if part of the scores group)
    sensitivity_scores = [f[f"clean_scores/epoch_{step}"]["sensitivities"][:] for step in range(total_steps)]
    bls = [f[f"clean_scores/epoch_{step}"]["bls"][:] for step in range(total_steps)]
    bpe = [f[f"clean_scores/epoch_{step}"]["bpe"][:] for step in range(total_steps)]
...
scaled_alphas_list, scaled_sizes_list = scaler(sensitivity_scores, args.scale_factor)

bls_alphas_list, bls_sizes_list = scaler(bls, args.scale_factor)

bpe_alphas_list, bpe_sizes_list = scaler(bpe, args.scale_factor)

shared_resource = ColumnDataSource(data={
    "step": list(range(len(sensitivity_scores))),
...
bls_resource = ColumnDataSource(data={
    "step": list(range(len(sensitivity_scores))),
    "alpha": bls_alphas_list,
    "size": bls_sizes_list,
    "xs": xs,
    "ys": ys,
})

bls_source = ColumnDataSource(data={
    "id": ids,
    "x": X_train[:, 0],  # First dimension of X_train
    "y": X_train[:, 1],  # Second dimension of X_train
    "class": y_train,  # Class labels
    "color": ['white'] * len(y_train),
    "marker": [marker[cls] for cls in y_train],
    "alpha": bls_alphas_list[0],
    "size": bls_sizes_list[0],
})

bpe_resource = ColumnDataSource(data={
    "step": list(range(len(sensitivity_scores))),
    "alpha": bpe_alphas_list,
    "size": bpe_sizes_list,
    "xs": xs,
    "ys": ys,
})

bpe_source = ColumnDataSource(data={
    "id": ids,
    "x": X_train[:, 0],  # First dimension of X_train
    "y": X_train[:, 1],  # Second dimension of X_train
    "class": y_train,  # Class labels
    "color": ['white'] * len(y_train),
    "marker": [marker[cls] for cls in y_train],
    "alpha": bpe_alphas_list[0],
    "size": bpe_sizes_list[0],
})
...
bpevisualizer = EvolvingBoundaryVisualizer(
    bpe_source,
    bpe_resource,
    len(sensitivity_scores),
    colors,
    1,
    max_steps=total_steps - 1,
    plotname='Prediction Error'
)

blsvisualizer = EvolvingBoundaryVisualizer(
    bls_source,
    bls_resource,
    len(sensitivity_scores),
    colors,
    1,
    max_steps=total_steps - 1,
    plotname='Leverage Score'
)

# Layout
boundary_layout = column(boundaryvisualizer.get_layout(), width=575, height=575)
bpe_layout = column(bpevisualizer.get_layout(), width=575, height=575)
bls_layout = column(blsvisualizer.get_layout(), width=575, height=575)

layout = row(boundary_layout, bpe_layout, bls_layout)
