...
def validate_label_server(h5_file):
    try:
        with h5py.File(h5_file, "r") as f:
            required_groups = ["images", "labels", "noise", "bpe", "bls", "config"]
            for group in required_groups:
                if group not in f:
                    print(f"Error: Missing required dataset '{group}' in the HDF5 file for label_server.")
                    return False
            if "config_data" not in f["config"]:
                print("Error: Missing 'config_data' in 'config' group for label_server.")
                return False
    except Exception as e:
        print(f"Error: Failed to read HDF5 file '{h5_file}' for label_server. {str(e)}")
        return False
    return True
...
def validate_h5_file(h5_file, project_type):
    ...
    elif project_type == "label_server":
        return validate_label_server(h5_file)
    else:
...
def main():
    ...
    parser.add_argument("--project", type=str, required=True, choices=["evolving_server", "mpe_server", "label_server"], help="Specify the project type")
